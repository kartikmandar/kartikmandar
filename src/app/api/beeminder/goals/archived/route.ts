...
export async function GET() {
  ...
  try {
    const endpoint = `${BEEMINDER_API_BASE}/users/${username}/goals/archived.json?auth_token=${encodeURIComponent(authToken)}`
    ...
    console.error('Failed to fetch Beeminder archived goals:', error)
    return NextResponse.json(
      { error: 'Failed to fetch Beeminder archived goals' },
      { status: 500 }
    )
  }
}