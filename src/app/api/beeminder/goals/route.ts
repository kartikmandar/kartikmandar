...
export async function GET() {
  ...
  try {
    const endpoint = `${BEEMINDER_API_BASE}/users/${username}/goals.json?auth_token=${encodeURIComponent(authToken)}`
    ...
    console.error('Failed to fetch Beeminder goals:', error)
    return NextResponse.json(
      { error: 'Failed to fetch Beeminder goals' },
      { status: 500 }
    )
  }
}